...
## Iteration 6 (2026-06) — Main agent notes
Peta Sebaran Asset: GET /api/public/catalog/map?keyword=&category_id=&provinsi=&kabupaten_kota=&kecamatan=&wilayah_level_4=&price_drop= -> {total, pins[{id,judul_asset,latitude,longitude,harga_limit,kategori,subkategori,kabupaten_kota,provinsi,image,is_sold,has_schedule,penurunan_persen}]} (only published w/ coordinates). Screen /map (app/map.tsx): Leaflet + markercluster in iframe/WebView; pin tap -> map-selected-card (map-card-open -> /asset/{id}, map-card-fav, map-card-close); legend; map-empty pill when no pins. Entry: home header map-entry-button and results pill view-on-map (passes active filters as params). Home header compacted (login button icon+label 48px).
Push notifications: user chose to postpone (not implemented).
